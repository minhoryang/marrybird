...
class OH_Question:
    ...
    @classmethod
    def loads(cls, filename=OH_JSON):
        OH_json = loads(open(filename).read())
        question_keys = list(map(int, OH_json.keys()))
        question_keys.sort()
        if not question_keys == list(range(1,21)):
            raise OH_Exception('Failed to load OH.json')

        questions = []
        for qk in question_keys:
            question = OH_json[str(qk)]
            mq = OH_Question()
            for key, value in question.items():
                if not value:
                    ...
                    raise OH_Exception('%s %s' % (key, value))
            mq.isDone()
            questions.append(mq)
        return questions

class OH_Exception(Exception):
    pass
